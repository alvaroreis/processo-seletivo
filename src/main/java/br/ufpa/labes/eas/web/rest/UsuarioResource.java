...
import br.ufpa.labes.eas.domain.Usuario;
import br.ufpa.labes.eas.repository.UsuarioRepository;
...
public class UsuarioResource {
    ...
    public List<Usuario> getAllUsuarios(@RequestParam(required = false) String filter) {
        if ("unidade-is-null".equals(filter)) {
            log.debug("REST request to get all Usuarios where unidade is null");
            return StreamSupport
                .stream(usuarioRepository.findAll().spliterator(), false)
                .filter(usuario -> usuario.getUnidade() == null)
